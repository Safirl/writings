...
const Planet = (props: PlanetProps) => {
  ...
  const [matcapTexture, setMatcapTexture] = useState<THREE.Texture | null>(
    null
  );
  ...
  useEffect(() => {
    if (!head.scene || !matcapTexture) return;

    // const matcapMaterial = new THREE.MeshMatcapMaterial({
    //     matcap: matcapTexture,
    // });

    const matcapMaterial = new THREE.MeshStandardMaterial({ color: "white" });
