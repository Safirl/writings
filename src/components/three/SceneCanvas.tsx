...
import { StatsGl, useProgress } from "@react-three/drei";
...
const SceneCanvas = (props: SceneCanvasProps) => {
  ...
  const [cameraAnimationObject, setCameraAnimationObject] =
    useState<animationObject>();
  const [enableOrbitControls, setEnableOrbitControls] = useState(true);
  const [currentCardTransform, setCurrentCardTransform] = useState<{
  ...
  useGSAP(() => {
    ...
    gsap.to(transitionObjectRef.current.style, {
      opacity: 0,
      duration:
        transitionTimers.find(
          (timer) => timer.key === "transitionObjectIntroIn"
        )?.value! / 1000,
      ease: "power2.inOut",
      delay: 1,
      onComplete: () => {
  ...
  const cardTransitionOut = () => {
    ...
    setCameraAnimationObject({
      ...
      targetRotation: new THREE.Euler(
        currentCardTransform?.rotation.x,
        currentCardTransform?.rotation.y,
        currentCardTransform.rotation.z
      ),
      delay: 0,
      duration: transitionTimers.find((timer) => timer.key === "projectOut")
        ?.value!,
      ease: "power2.inOut",
      onComplete: () => {
  ...
  const getTransitionObjectDelay = () => {
    if (currentCardId != -1) {
      return transitionTimers.find(
        (timer) => timer.key === "transitionObjectDelayIn"
      )?.value;
    }
    return transitionTimers.find(
      (timer) => timer.key === "transitionObjectDelayOut"
    )?.value;
  };
  ...
  const getTransitionObjectDuration = () => {
    if (currentCardId != -1) {
      return (
        transitionTimers.find(
          (timer) => timer.key === "transitionObjectDurationIn"
        )?.value! / 1000
      );
    }
    return (
      transitionTimers.find(
        (timer) => timer.key === "transitionObjectDurationOut"
      )?.value! / 1000
    );
  };
      ...
      <Suspense fallback={null}>
        ...
        >
          ...
          <Environment
            ...
            isCardClicked={currentCardId != null && currentCardId != -1}
            transitionTimers={transitionTimers}
          />
